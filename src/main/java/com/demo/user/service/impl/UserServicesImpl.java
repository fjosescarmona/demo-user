...
public class UserServicesImpl implements UserServices {
  ...
  public Mono<UserResponseDto> createUser(UserRequestDto userRequestDto) {
    ...
    return userRepository
        .save(entityMapper.userRequestDtoToUserEntity(userRequestDto))
        .flatMap(
            user -> {
              List<PhoneEntity> phoneEntities =
                  userRequestDto.getPhones().stream()
                      .map(
                          phone ->
                              entityMapper.userRequestDtoToPhoneEntity(phone, user.getUserId()))
                      .collect(Collectors.toList());

              return phoneRepository
                  .saveAll(phoneEntities)
                  .collectList()
                  .map(savedPhones -> entityMapper.userEntityToUserResponseDto(user, savedPhones))
                  .onErrorResume(
                      error -> Mono.error(new GenericErrorResolver("Error Creating user")));
            })
        .onErrorResume(
            error ->
                Mono.error(
